...
function test()
    ...
    @test norm(
        [solnestim, beta, c] - [92.46031652777476, 0.6053628424093497, -2.471055221256022],
    ) < 1.e-5

    sol = [231.7, 239.1, 244.8]
    h = [(4.0 / 5.0)^i for i in 0:1:2]
    solnestim, beta, c, residual = AlgoBaseModule.richextrapol(sol, h)
    # # println("$((solnestim, beta, c))")
    @test norm(
        [solnestim, beta, c] - [263.91176470588067, 1.1697126080157385, 32.21176470588068],
    ) < 1.e-5
...
function test()
    ...
    @test norm(sort(partitionnumbers) - sort([
        1
        3
        2
        5
...
function test()
    H = 30.0 # strip width
    R = 10.0 # radius of the hole
    L = 20.0 # length of the strip
    nL = 15
    nH = 10
    nR = 5
...
function test()
    ...
    @test norm(u.values - geom.values) < 1.0e-5
    wipe!(u)
    numberdofs!(u, count(fens):-1:1)
    @test isapprox(vec(u.dofnums[1, :]), vec([178 179 180]))
    # @show u
    @test norm(u.dofnums) > 1.0e-3
    wipe!(u)
    @test norm(u.dofnums) == 0
...
function test()
    ...
    Degree = [length(findall(x -> x != 0, C[j, :])) for j in axes(C, 1)]
...
function test()
    ...
    hs = [(4.0 / 5.0)^i for i in range(0, length = 3)]
...
function test()
    ...
    testsets = [
        (alpha = 0.5, beta = 2.0, c = 0.3, truesol = 1.0),
...
function test()
    ...
    testsets = [
        (parameters = [0.5, 0.3, 0.1], beta = 2.0, c = 0.3, truesol = 1.0),
...
function test()
    ...
    @test abs.(
        maximum(
            [
                0.833404 0.599773 0.460794 0.0512104 0.24406 0.254868 0.476189
...
function test()
    ...
    modeldatacoarse = Dict{String,Any}(
        ...
        "geom" => geom,
        "elementsize" => Ly / ny,
        "geometricaltolerance" => Ly / ny / 100,
        "parametrictolerance" => 1.0e-3,
    )
    ...
    modeldatafine = Dict{String,Any}(
        ...
        "geom" => geom,
        "elementsize" => Ly / ny / 2,
        "geometricaltolerance" => Ly / ny / 2 / 100,
        "parametrictolerance" => 1.0e-3,
    )
...
function test()
    ...
    modeldatafinest = Dict{String,Any}(
        ...
        "geometricaltolerance" => Ly / ny / 2 / 2 / 100,
        "parametrictolerance" => 1.0e-3,
    )
...
function test()
    ...
    result = VTK.vtkexportmesh(
        ...
        fens,
        fes;
        scalars = [("psi", psi.values), ("psih", psih.values)],
    )
    @test result == true
    rm(File)
    true
...
function test()
    ...
    psih = fieldfromintegpoints(femm, geom, u, dT, :psi, 1; nodevalmethod = :averaging)
    @test nnodes(psih) == count(fens)
    ...
    psih = fieldfromintegpoints(femm, geom, u, dT, :psi, 1; nodevalmethod = :invdistance)
    @test nnodes(psih) == count(fens)
...
mutable struct FESetYEtAnother end
