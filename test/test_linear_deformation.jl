...
function test()
    ...
    OmegaShift = (10.0*2*pi)^2;
    stabfact = 0.005
    Eigenvalues =   [0.0, 0.0, 3.86052e-5, 6.16182e-5, 8.47028e-5, 0.000106305, 2490.71, 2491.92, 2503.84, 4083.33, 4568.84, 4570.35, 4962.2, 6613.57, 6614.4, 6616.47, 6617.7, 6712.47, 6714.77, 6775.52]
    MR = DeforModelRed3D
    output = import_ABAQUS("alum_cyl.inp")
    fens, fes = output["fens"], output["fesets"][1]
    fens.xyz .*= phun("mm") # The input is provided in SI(mm) units
...
function test()
    ...
    if true
        ...
        fs = real(sqrt.(complex(d)))/(2*pi)
        println("Eigenvalues: $fs [Hz]")
        @test norm(fs .- [0.0, 0.0, 0.0, 4.50258e-5, 8.83575e-5, 9.64245e-5, 410.658, 464.14, 584.025, 604.455, 1152.1, 1152.1, 1365.7, 1365.7, 1972.9, 2022.33, 2197.46, 2198.46, 2214.14, 2219.45]) < 0.001 * norm(fs)
        # mode = 7
        # scattersysvec!(u, v[:,mode])
