...
f2l=selectelem (fens,bfes,box=[-Inf,Inf,-Inf,Inf,-angslice,-angslice],inflate=tolerance);
...
list=selectnode(fens,distance=1.0+0.1/2^nref, from=[0. 0. 0.], inflate=tolerance);
fens.xyz[list,:]= JFinEALE.MeshUtilModule.ontosphere(fens.xyz[list,:],1.0);
...
l1 =selectnode(fens,box=JFFlt[-Inf  Inf -Inf  Inf 0.0 0.0],inflate=tolerance);
l2 =selectnode(fens,distance=1.0+0.1/2^nref,from=JFFlt[0.0 0.0 0.0],inflate=tolerance);
nA=intersect(l1,l2);
