...
botskinregion = FDataDict("femm"=>FEMMDeforLinear(MR,
    IntegData(subset(fes, rls), gr, CSys(3, 3, updatecs!)), skinmaterial))
rls = selectelem(fens, fes, label = 3)
topskinregion = FDataDict("femm"=>FEMMDeforLinear(MR,
    IntegData(subset(fes, rls), gr, CSys(3, 3, updatecs!)), skinmaterial))
rlc = selectelem(fens, fes, label = 2)
coreregion = FDataDict("femm"=>FEMMDeforLinear(MR,
...
Trac = FDataDict("traction_vector"=>vec([0.0; 0.0; -q0]),
    "femm"=>FEMMBase(IntegData(subset(bfes, tl), GaussRule(3, 3))))
