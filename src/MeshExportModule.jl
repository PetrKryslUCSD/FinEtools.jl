...
# function export_NASTRAN(filename::AbstractString, ddict::FDataDict)
#     open(filename, "w") do fid
#         @printf fid "\$ Generated by FinEtools\n"
#         @printf fid "BEGIN BULK\n"
#         fens = ddict["fens"]
#         for i = 1:count(fens)
#             @printf fid "GRID,%d,,%g,%g,%g\n" i fens.xyz[i,1] fens.xyz[i,2] fens.xyz[i,3]
#         end
#         fesets = ddict["fesets"]
#         e = 1
#         for j = 1:length(fesets)
#             elname = NASTRANtypemap[typeof(fesets[j])]
#             conn = fesets[j].conn
#             nc = size(conn, 2)
#             for i = 1:size(conn, 1)
#                 @printf fid "%s,%d,%d" elname e j
#                 for k = 1:nc
#                     @printf fid ",%d" conn[i, k]
#                     if k == 6
#                         @printf fid "\n"
#                     end
#                 end
