...
in the correct size
"""
function gathervalues_asvec!(self::F,
    dest::AbstractArray{T, 1},
    conn::CC) where {F <: AbstractField, T, CC}
...
in the correct size
"""
function gathervalues_asmat!(self::F,
    dest::AbstractArray{T, 2},
    conn::CC) where {F <: AbstractField, T, CC}
...
in the correct size
"""
function gatherfixedvalues_asvec!(self::F,
    dest::AbstractArray{T, 1},
    conn::CC) where {F <: AbstractField, T, CC}
...
in the correct size
"""
function gatherfixedvalues_asmat!(self::F,
    dest::AbstractArray{T, 2},
    conn::CC) where {F <: AbstractField, T, CC}
    Zer = zero(eltype(self.values))
    for j in axes(self.values, 2)
...
function _setebc!(self::F,
    fenid::IT,
    is_fixed::Bool,
    comp::IT,
    val::T) where {F <: AbstractField, T <: Number, IT <: Integer}
...
function setebc!(self::F,
    ...
    is_fixed::Bool,
    comp::IT,
    val::AbstractVector{T}) where {F <: AbstractField, T <: Number, IT <: Integer}
...
function setebc!(self::F,
    ...
    is_fixed::Bool,
    comp::IT,
    val::T = 0.0) where {F <: AbstractField, T <: Number, IT <: Integer}
...
"""
function setebc!(self::F,
    fenids::AbstractVector{IT}) where {F <: AbstractField, IT <: Integer}
    zer = zero(eltype(self.values[1]))
...
degrees of freedom.
"""
function scattersysvec!(self::F,
    vec::AbstractVector{T}) where {F <: AbstractField, T <: Number}
    nents, dim = size(self.values)
    nve = length(vec)
    for i in 1:nents
        for j in 1:dim
...
degrees of freedom.
"""
function incrscattersysvec!(self::F,
    vec::AbstractVector{T}) where {F <: AbstractField, T <: Number}
    nents, dim = size(self.values)
    nve = length(vec)
    for i in 1:nents
