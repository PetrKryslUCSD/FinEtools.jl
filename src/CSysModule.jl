...
where
`csmatout`= output matrix buffer, `XYZ`= location  in physical coordinates,
...
Construct coordinate system when the
...
`mdim` = number of manifold dimensions of the finite element in which the coordinate
...
Compute the coordinate system  for an isotropic material using
information  available  by looking at the coordinate curves of isoparametric
finite elements.
...
an mdim-dimensional manifold element, which is embedded in a
sdim-dimensional Euclidean space. If mdim == sdim,
the coordinate system matrix is the identity; otherwise the local
