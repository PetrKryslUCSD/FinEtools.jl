...
function mass(self::FEMMDeforLinearAbstract,  assembler::A,  geom::NodalField{FFlt}, u::NodalField{T}) where {A<:SysmatAssemblerBase, T<:Number}
    ...
    startassembly!(assembler,  size(elmat,1),  size(elmat,2),  count(geod.fes), u.nfreedofs,  u.nfreedofs);
    ...
    for i = 1:count(geod.fes) # Loop over elements
        ...
        if norm(DeltaT) != 0     # Is the thermal increment nonzero?
            ...
            for j = 1:npts # Loop over quadrature points
                ...
                thstress = self.material.update!(self.material, thstress, thstress, strain, thstrain, t, dt, loc, geod.fes.label[i], :nothing)
...
function inspectintegpoints(self::FEMMDeforLinearAbstract, geom::NodalField{FFlt},  u::NodalField{T}, dT::NodalField{FFlt},
    ...
    for ilist = 1:length(felist) # Loop over elements
        ...
        for j = 1:npts # Loop over quadrature points
            ...
            out = self.material.update!(self.material, qpstress, out, vec(qpstrain), qpthstrain, t, dt, loc, geod.fes.label[i], quantity)
